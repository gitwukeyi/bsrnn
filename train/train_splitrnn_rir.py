...
class DdpTrainer:
    def __init__(self, model: Module, gpu_id: int, configs: train_config):
        ...
        self.lr_scheduler = CosineAnnealingWarmRestarts(optimizer=self.optimizer, T_0=3, T_mult=1)
        self.iters = 0
        self.current_epoch = 0

        self.model = model.to(gpu_id)  # model should move to gpu first
    ...
    def loss(self, y_pred: Tensor, y_true: Tensor):
        ...
        :return:
        """
        y_pred = torch.clip(y_pred, min=-1, max=1)
    ...
    def run_epoch(self, dl: DataLoader, training: bool = True):
        ...
        for idx, batch_data in loop:
            if training is True:
                ...
                self.lr_scheduler.step(self.current_epoch + idx/self.iters)
    ...
    def train(self, train_dl: DataLoader, val_dl: DataLoader):
        ...
        for idx in range(self.epoch):
            ...
            torch.save(self.model.state_dict(), f=path)  # save last model

            self.log_writer.add_scalar("loss/val", np.round(val_loss, 4), global_step=idx)
...
def main(rank,
    ...
    checkpoint = torch.load(r"/data3/wukeyi/code/attention_lstm_se/checkpoint/best_0.0538.pth.rar")
    ...
    train_loader, val_loader = g_build_loader(
        ...
        snr=configs.train_snr, multi_gpu=False, load_pre_data=True, pre_data_pkl_path=configs.pre_data_pkl_path
    )
