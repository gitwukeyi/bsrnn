...
def compute_metrics(data1, data2, Fs=16000):
    alpha = 0.95
...
def llr(clean_speech, processed_speech,sample_rate):
    ...
    for frame_count in range(num_frames):
        ...
        distortion[frame_count] = math.log(numerator / (denominator+1e-9) + 1e-9)
        start = start + skiprate
    return distortion
...
def lpcoeff(speech_frame, model_order):
    ...
    for i in range(model_order):
        ...
        rcoeff[i] = (R[i + 1] - sum_term) / (E[i] + 1e-9)
        a[i] = rcoeff[i]
        if i == 0:
...
def removeSilentFrames(x, y, dyrange, N, K):
    ...
    msk = 20 * np.log10(np.divide(norm(np.multiply(x[jj_list], w), axis=1), np.sqrt(N)) + 1e-9)
